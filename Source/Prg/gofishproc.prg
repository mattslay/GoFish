...
* --------------------------------------------------------------------------------
Procedure GF_RemoveFolder(lcFolderName)
		...
		For lnI = 1 To m.lnFileCount
			...
			If Left(m.lcFileName, 1) # '.'
				...
				If 'D' $ m.laFiles[m.lnI, 5] && directory?
					GF_RemoveFolder(m.lcFileNameWithPath)
				Else
...
Return 	GF_RtlS2PL(@m.tnDest, @m.tcSrc, m.tnLen)
...
*get location for GoFish from ResourceFile
	...
	llFound = GF_Create_LocalPath(@lcFolder,m.llFound,toSettings.lCR_Local_Default,toSettings.lCR_Local_Default)
...
Procedure GF_Change_TableStruct	&&Update structur of storage tables from version pre 6.*.*
Lparameters;
	toResultForm,;
	tcRoot,;
...
Try
		...
		If !File(Addbs(m.lcDir) + 'README.md') Then
			Strtofile(GF_Get_Readme_Text(2),Addbs(m.lcDir) + 'README.md')
...
Procedure GF_Get_Readme_Text  	&&Text for README.md Files
Lparameters;
	tnFile
